class ScreenSide extends Phaser.Scene {
    constructor() {
    ...
    create() {
        ...
        //key2
        if (!gameState.items.key2.found) {

            this.key2Layer = this.add.image(0, 0, 'key2Layer').setOrigin(0, 0);

            this.key2HitArea = this.add.graphics();
            const key2HitAreaPoints = [420, 200, 420, 280, 458, 280, 455, 200];

            this.setHitArea(this.key2HitArea, key2HitAreaPoints, false);

            this.key2HitArea.on('pointerdown', function() {
                this.getItem('key2');
                this.key2Layer.destroy();
                this.key2HitArea.destroy();
            }, this);
        ...
        //Power
        this.power = this.add.image(0, 0, 'power').setOrigin(0, 0);
        this.powerPressed = this.add.image(0, 0, 'powerPressed').setOrigin(0, 0);
    
        this.powerHitArea = this.add.graphics();
        const powerHitAreaPoints = [435, 158, 20];

        this.setHitArea(this.powerHitArea, powerHitAreaPoints, false);

        this.powerHitArea.on('pointerdown', function() {
            this.power.setVisible(false);
            this.powerPressed.setVisible(true);
        }, this);

        this.powerHitArea.on('pointerout', function() {
            this.powerPressed.setVisible(false);
            this.power.setVisible(true);
        }, this);

        this.powerHitArea.on('pointerup', function() {
            this.powerPressed.setVisible(false)
            this.power.setVisible(true);
            gameState.doneEvents.screen.on = true;
        }, this);
        
        
        this.itemBarConstructor();
        this.navBelowArrowBConstructor('screenSide', 'GameScene3');
