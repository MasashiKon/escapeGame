class Shed1 extends Phaser.Scene {
    constructor() {
        super({key: 'Shed1'})
    }
    ...
    create() {
        this.add.image(0, 0, 'shed').setOrigin(0, 0);
        //this.add.image(0, 0, 'shed1Num').setOrigin(0, 0);

        if (!gameState.items.bottle.found) {
            this.bottleLayer = this.add.image(0, 0, 'shed1BottleLayer').setOrigin(0, 0);

            this.bottleLayerHitArea = this.add.graphics();
            const bottleLayerHitAreaPoints = [350, 90, 490, 80, 490, 230, 550, 280, 550, 440, 310, 440, 310, 280, 360, 230];

            this.setHitArea(this.bottleLayerHitArea, bottleLayerHitAreaPoints, false);

            this.bottleLayerHitArea.on('pointerdown', function() {
                this.getItem('bottle');
                this.bottleLayer.destroy();
                this.bottleLayerHitArea.destroy();
            }, this)
        }
        
        this.itemBarConstructor();
        this.navBelowArrowBConstructor('Shed1', 'GameScene1');
