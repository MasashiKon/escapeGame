class GameSceneDrawer1Upside extends Phaser.Scene {
    constructor() {
    ...
    create() {
        ...
        // pcCable
        if(!gameState.items.screenControler.found) {
            this.pcCable = this.add.image(0, 0, 'screenControlerLayer').setOrigin(0, 0);
            this.pcCableHitArea = this.add.graphics();
            const pcCableHitAreaPoints = [420, 30, 540, 130, 300, 350, 220, 220];
            
            /*
            const pcCable = this.add.image(0, 0, 'pcCableLayer').setOrigin(0, 0);
            const pcCableHitArea = this.add.graphics();
            pcCableHitArea.fillStyle(0x000000).fillRect(225, 50, 375, 360);
            pcCableHitArea.setVisible(false);
            pcCableHitArea.setInteractive(new Phaser.Geom.Rectangle(225, 50, 375, 360), Phaser.Geom.Rectangle.Contains);
            pcCableHitArea.input.alwaysEnabled = true;

            pcCableHitArea.on('pointerdown', () => {
                gameState.items.pcCable.found = true;
                gameState.itemBar.push('pcCable');
                pcCable.destroy();
                pcCableHitArea.destroy('GameSceneDrawer1Upside', 'GameScene2');
            });
            */

            this.setHitArea(this.pcCableHitArea, pcCableHitAreaPoints, false);

            this.pcCableHitArea.on('pointerdown', function() {
                this.getItem('screenControler');
                this.pcCable.destroy();
                this.pcCableHitArea.destroy();
            }, this);

        }

        this.navBelowArrowBConstructor('GameSceneDrawer1Upside', 'GameScene2');
        this.itemBarConstructor(this.pcCableHitArea, );
