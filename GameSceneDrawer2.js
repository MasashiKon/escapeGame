class GameSceneDrawer2 extends Phaser.Scene {
    constructor() {
        super({key: 'GameSceneDrawer2'})
    }
    ...
    create() {
        ...
        if (!gameState.items.bananas.found) {
            this.handleLayer = this.add.image(0, 0, 'bananaLayer').setOrigin(0, 0);

            this.handleLayerHitArea = this.add.graphics();
            const handleLayerHitAreaPoints = [350, 50, 500, 65, 520, 300, 440, 410, 220, 430, 220, 300, 300, 230];

            this.setHitArea(this.handleLayerHitArea, handleLayerHitAreaPoints, false);

            this.handleLayerHitArea.on('pointerdown', function() {
                this.getItem('bananas');
                this.handleLayer.destroy();
                this.handleLayerHitArea.destroy();
            }, this);          
        }
