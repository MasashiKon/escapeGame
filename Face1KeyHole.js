class Face1KeyHole extends Phaser.Scene {
    constructor() {
    ...
    create() {
        ...
        //Key2 action
        if (!gameState.doneEvents.face1Door.keyInserted && !gameState.doneEvents.face1Door.turned) {

            this.key2DropArea = this.add.graphics();
            const key2HitAreaPoints = [390, 315, 150];

            this.setHitArea(this.key2DropArea, key2HitAreaPoints, false, true, true);

            this.input.on('drop', function(pointer, gameObject, dropZone) {
                if(gameState.grabedItem.id === 'key3') {
                    this.itemDone('key3');
                    this.key2DropArea.destroy();
                    this.keyInserted = this.add.image(0, 0, 'face1KeyInserted').setOrigin(0, 0);
                    gameState.doneEvents.face1Door.keyInserted = true;
                    
                    this.key2HitArea = this.add.graphics();
                    const key2HitAreaPoints = [320, 180, 460, 180, 460, 470, 320, 470];

                    this.setHitArea(this.key2HitArea, key2HitAreaPoints, false);

                    this.key2HitArea.on('pointerup', function() {
                        this.keyInserted.destroy();
                        this.key2HitArea.destroy();
                        this.add.image(0, 0, 'face1KeyTurned').setOrigin(0, 0);
        ...
        } else if (gameState.doneEvents.face1Door.keyInserted && !gameState.doneEvents.face1Door.turned) {
            this.keyInserted = this.add.image(0, 0, 'face1KeyInserted').setOrigin(0, 0);
        ...
        this.navBelowArrowBConstructor('Face1KeyHole', 'GameScene1');
