class MixScene extends Phaser.Scene {
    ...
    create() {
        ...
        gameState.doneEvents.glassCup.isDone = true;
       
        this.add.image(0, 0, 'controlPanel').setOrigin(0, 0);
        this.handleSound = this.sound.add('handleSound');
        this.add.image(0, 0, 'controlPanelKeyHoleTurned').setOrigin(0, 0);
        
        this.handleSMixprite = this.add.sprite(0, 0, 'handleSpriteSheet').setOrigin(0, 0);
        ...
        this.handleSMixprite.anims.play('handleMixAction', true);
        ...
        const handleSoundRepeat = setInterval(() => {
            ...
            if(handleSoundCount >= 20) {clearInterval(handleSoundRepeat)};

        }, 510);

        const mixingSound = setTimeout(() => {

            this.mixStart = this.sound.add('mixStart');
            this.mixStart.play();

            const mixingEndSound = setTimeout(() => {

                this.mixEnd = this.sound.add('mixEnd');
                this.mixEnd.play();

                this.mixStart.stop();

                this.itemBarConstructor();
                this.navBelowArrowBConstructor('MixScene', 'GameScene3');
